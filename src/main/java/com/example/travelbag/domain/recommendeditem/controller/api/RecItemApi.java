...
public interface RecItemApi {
    ...
    @Operation(summary = "추천 준비물 조회", description = "회원 및 가방 정보를 기반으로 추천 준비물을 조회합니다.")
    @ApiResponses({
    ...
    ResponseEntity<List<RecItemResponseDto>> getRecommendedItems(
            ...
            @Parameter(description = "준비물 카테고리", required = true) ItemCategory category
    );

    @Operation(summary = "추천 준비물 추가", description = "추천 준비물을 가방에 추가합니다.")
    @ApiResponses({
